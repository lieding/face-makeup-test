...
def main():
    ...
    for item in tqdm(data, desc="Extracting face embeddings"):
        ...
        faceid_embeds_list_.append(faceid_embeds)
    ...
    for item in tqdm(data, desc="Extracting face keypoints"):
        ...
        face_kps_list_.append(face_kps)
    ...
    for item in tqdm(data,desc="Extracting faces_list"):
        ...
        faces_list_.append(faces)
        # faces_list_.append(faces if not resize else faces.resize((512, 512)))
    ...
    for item in data:
       for idx, prompt_text in enumerate(prompts):
            try:
                ...
                faceid_embed = faceid_embeds_list[filename]
                # 平均
                # faceid_embeds = torch.mean(torch.stack(faceid_embeds_list_), dim=0)

                faceid_embeds = torch.stack(faceid_embeds_list_)
                alpha = 0.5
                beta = 1 - alpha
                faceid_embeds =  faceid_embeds[0] * alpha + faceid_embeds[1] * beta
                
                # 调用模型生成图像
                images = model.generate(
                    prompt=prompt,  # 使用每次的新 prompt
                    negative_prompt="black image",
                    image=face_kps,             # antelopev2
                    face_image=faces_list_,           # buffalo_l
