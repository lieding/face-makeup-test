...
class FaceMakeUp:
    ...
    def __init__(self, sd_pipe, image_encoder_path, ip_ckpt, device, lora_rank=128, num_tokens=4, torch_dtype=torch.float16 , controlnet_ckpt = None):
        self.device = device
        ...
        # self.controlnet = self.init_controlnet()
    ...
    def load_ip_adapter(self):
        ...
        if self.controlnet_ckpt is not None:
            model_path = self.controlnet_ckpt
    ...
    def get_image_embeds(self, faceid_embeds, face_image, s_scale, shortcut, mix=False):
        ...
        if mix:
            ...
            # 平均
            # clip_image_embeds = torch.mean(clip_image_embeds, keepdim=True, dim=0)
            # uncond_clip_image_embeds = torch.mean(uncond_clip_image_embeds, keepdim=True,dim=0)
    ...
    def generate(
        ...
        **kwargs,
        ...
        image_prompt_embeds, uncond_image_prompt_embeds = self.get_image_embeds(faceid_embeds, face_image, s_scale, shortcut, mix = mix)
